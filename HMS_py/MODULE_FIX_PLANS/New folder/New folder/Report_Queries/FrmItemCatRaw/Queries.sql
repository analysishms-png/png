...
Select SubCode As Code,Name From ViewSubGroup
    Where ActiveYN=1 and (LOGSITE_CODE='<LogSiteCode>' or LOGSITE_CODE='HO') AND Left(MainGrCodeS,3) IN ('230','240','250','260','270','280','040')
    Order by Name;
...
-- --------------------------------------------
Select SubCode As Code,Name From ViewSubGroup
    Where ActiveYN=1 and (LOGSITE_CODE='<LogSiteCode>' or LOGSITE_CODE='HO') AND Left(MainGrCodeS,3) IN ('230','240','280','040')
    Order by Name;
...
-- Uses table: ViewSubGroup
-- --------------------------------------------
SELECT distinct IC.Code,IC.Name AS ItemCat,ShortName,IC.RoundOff,IC.AcName,IC.TaxStru As TaxStruCode,IC.Flag,IC.CatType,IC.RestCode,IC.TaxStru,IC.TaxStruType,IC.U_Name As IUName,VS.Name As SaleAc,TaxStru.Name As TaxStructure,D.Name As RestName,IC.Drcr,IC.SITE_CODE,IC.LOGSITE_CODE FROM ((ItemCatMast IC
    ...
    where (IC.LOGSITE_CODE='<LogSiteCode>' or IC.LOGSITE_CODE='HO') and resttype='Store'
...
Select Code,Name,ShortName From Depart
    Where (LOGSITE_CODE='<LogSiteCode>' or LOGSITE_CODE='HO') and RestType='Store'
    Order by Name;
...
SELECT IC.Code,IC.Name AS ItemCat,IC.RoundOff,IC.Flag,IC.CatType As Type,D.Name As Outlet,VS.Name As SaleAc,TaxStru.Name As TaxStructure,D.Name as OutLetName FROM ((ItemCatMast IC
    ...
    Left Join Depart D On IC.RestCode=D.Code
    WHERE (IC.LOGSITE_CODE='<LogSiteCode>' or IC.LOGSITE_CODE='HO') AND d.code='<LogSiteCode>PURC'
    ORDER BY IC.Name;

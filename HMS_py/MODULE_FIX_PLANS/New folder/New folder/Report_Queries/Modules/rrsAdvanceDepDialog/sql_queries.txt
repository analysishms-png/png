...
-- Uses table: Waiter
-- --------------------------------------------
SELECT Sale1.Docid,Sale1.Vno,sale1.NetAmt FROM ((Sale1
    LEFT JOIN PayCharge ON Sale1.DocId = PayCharge.DocId)
    LEFT JOIN RoomMast AS T ON (Sale1.RoomType = T.Type) AND (Sale1.RoomNo = T.Code) AND (Sale1.RestCode = T.RestCode))
    LEFT JOIN Waiter on SAle1.Waiter=Waiter.Code
    WHERE (SALE1.LOGSITE_CODE='<LogSiteCode>;
...
SELECT DP.PayCode as code, RevMast.name as Name, revMast.PayType as PayType FROM DepartPay as DP LEFT OUTER
    ...
    where (DP.LOGSITE_CODE='<LogSiteCode>') AND DP.restcode='<restcode>' And RevMast.Code Not In ('<LogSiteCode>CCAD')
    ORDER BY NAME;

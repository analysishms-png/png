...
/* ==== TRANSFER SCRIPT (patched, mode=filter) ==== */
/* ============================================================================
...
DECLARE @SRC_DB  sysname = N'TestSrcDB';   -- << EXISTING database ka naam
DECLARE @DEST_DB sysname = N'TestDstDB2';    -- << NEW database ka naam
...
DECLARE @ONLY_TABLES nvarchar(max) = N'Region, Sale*, Customer';
/* ========================================================================== */
...
CREATE TABLE #filter (name sysname PRIMARY KEY);
...
BEGIN
    SET @sql = N'CREATE DATABASE ' + @dst_db_q + N';';
    EXEC (@sql);
    PRINT 'STEP 1 : Database [' + @DEST_DB + '] CREATE ho gayi.';
END
ELSE
...
CREATE TABLE #tabs (name sysname PRIMARY KEY);
...
SET @sql =
    ...
    + N' LEFT JOIN (SELECT tt.name, SUM(pp.rows) AS DstCnt'

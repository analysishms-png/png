...
class BuildingMasterExport {
    ...
    public function getData() {
        return DB::table('buildingmaster')->where('propertyid', $this->propertyid)->orderBy('name', 'ASC')->get();
    }
    public function download() {
        $data = $this->getData();
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Building Master');
        ...
        $sheet->mergeCells('A2:C2'); $sheet->setCellValue('A2', 'Building Master');
        ...
        foreach ($data as $i => $row) {
            ...
            $sheet->setCellValue('C' . $currentRow, ($row->activeyn ?? 0) == 1 ? 'Yes' : 'No');
        ...
        header('Content-Disposition: attachment; filename="Building_Master.xlsx"');

...
| 12_Messaging | ✅ L1 413 / L2b 27 PASS (first run transient KI-12 red) | ✅ 2 items, failed=[], 2 ok | KI-10 classify norm-collision (FIXED 2026-09-25); KI-1 `EmailSMSForward MAX(Id)`/`DBSendSMS.DocId`; `SMSEnviro` singleton no-lock |
...
| KI-10 | classify norm-collision (`_norm_caption`) | `menu_help._norm_caption` trailing `(...)` strip (menu_help.py:35-38) → `SMS (API)/(Scheduled)/(Conditional)/SMS` sab `"sms"` normalize → curated masters leaf (`12_Messaging`) ke `master_caps` poore family ko masters me kheench leta tha (masters-first order) — ops bucket 0 despite curated ops leaf. Capture/registry_hits unaffected. **FIXED 2026-09-25:** `collect_logic.classify` me (a) paren-preserving exact `caption_disp` match curated leaves ke against PEHLE, (b) paren-leaf stripped norms legacy `master_caps`/`oper_caps` se excluded → 12 counts 5/0/0/0/1 → 2/1/0/0/3; other 13 modules zero change (regen diff); TDD `test_classify_norm_collision_exact_precedence` (suite 15→16 green) | fixed in tool (collect_logic.classify) — regen + docs updated |
...
| KI-14 | Concurrent workstream ke 14:21 push ne tracked sidebar test toda | `50627a3 "push"` (2026-09-25 14:21:15) ne `HMS_py/ui/shell.py` se `mod_target` property + `_on_sidebar_click` API REMOVE kar diya (sidebar ab `menu.sidebar_modules()` + `_on_module` style) — `tests/unit/test_menuhelp_sidebar_live.py::test_sidebar_click_menubar_menuhelp_sa` (tracked, untouched since 06:22 commit `24e3c44`) ab deterministic FAIL: `sidebar button missing: finance`. Evidence: `git log -S mod_target -- shell.py` → removal in 50627a3; 13:27/13:32 L1 = 434 passed (pre-push green); current L1 = 481 passed + 1 failed (ye wala). Hamari KI-10 changes se zero overlap (tools/docs only). Fix unke refactor ke saath test update karna — unka WIP, humne touch nahi kiya | cross-session breakage (their commit) — documented, relay to their session |

## Baseline numbers (2026-09-25, batch rollup)

- L1 unit: **434 tests** green at 13:32 (334→434 grew during day — pipeline ke naye tests + concurrent workstream ke desktop-surface tests; per-module matrix rows me historical counts). Post-batch status: **482 collected → 481 passed + 1 failed** (concurrent session ke 14:21 push ne 48 naye tests + shell sidebar API change kiye; 1 fail = KI-14, unka tracked test, hamare changes se zero overlap; hamara KI-10 test shamil = test_manual_pipeline 16 green)
...
- `test_manual_pipeline`: 16 green (capture_leaf opener try/except TDD test + KI-10 `test_classify_norm_collision_exact_precedence` shamil)
- Citation drift note: manuals me ~166 `shell:NNN` refs = batch-time snapshot (10:00-13:40); concurrent session ke 14:21 push + ongoing edits (shell.py mtime 15:27) se line numbers shift (spot-check: 14 Market Segment 1124→1105, 11 EPABX_Data 1425→1366). Registry captions intact (648 keys, pipeline leaves resolve) — capture unaffected; bulk citation update shell.py stabilize hone ke baad (policy: `00_Instructions/MANUAL_TEMPLATE.md` citation note)

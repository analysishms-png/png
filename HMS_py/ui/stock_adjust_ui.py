"""Stock Adjustment Entry (VB6: Stock Adjustment port)."""
...
class StockAdjustWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        hdr = QGroupBox("Adjustment Header")
        ...
        lines_grp = QGroupBox("Adjustment Lines (+Qty = Receive, -Qty = Issue)")
        ...
        self.btn_add.setToolTip("Add a new adjustment line")
        self.btn_del = QPushButton("Remove (-)")
        self.btn_del.setToolTip("Remove the selected adjustment line")
        self.btn_save = QPushButton("Save Adjustment")
        self.btn_save.setProperty("success", True)
        self.btn_save.setToolTip("Save the stock adjustment")
        self.btn_exit = QPushButton("Exit")
        self.btn_exit.setToolTip("Close the stock adjustment form")
    ...
    def _save(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            lines.append({"item": item, "godown": godown, "qty": qty,
                          "unit": "", "rate": rate, "amount": abs(qty) * rate})
        if not lines:
            QMessageBox.warning(self, "Empty", "Koi lines nahi"); return
        try:
            result = inv.stock_adjust(lines, vdate=vdate)
            QMessageBox.information(self, "Saved", "Stock Adjustment saved!\nDocId: %s" % result["docid"])
            self.table.setRowCount(0)
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

def open_stock_adjust(parent=None):
    w = StockAdjustWindow(parent); w.show(); return w

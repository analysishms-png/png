...
children.push(
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: cfg.property || "Property Analysis Demo (103)", size: 24, bold: true })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: cfg.version || "Version 2026-27", size: 22 })] }),
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync(cfg.output, buf);
  console.log("Written:", cfg.output, "(" + Math.round(buf.length / 1024) + " KB)");
});

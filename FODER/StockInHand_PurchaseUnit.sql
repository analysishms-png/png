-- =====================================================
-- STOCK IN HAND (Stores) - PURCHASE UNIT MODE (ConvRatio)
-- 01/04/2025 se aaj tak, Item Group wise, 0 stock bhi included
-- Site: ss
-- NOTE: Wt. Unit wali query se farq sirf yeh hai ki
--       quantities S.ConvRatio se multiply hain.
-- =====================================================
...
Opening AS (
    SELECT S.Item,
           SUM(CASE WHEN VT.NCAT IN ('PBC','PBR','STOP','MRE','RQI','KSREC','KMREC')
                    THEN S.QtyRec * ISNULL(S.ConvRatio, 1) ELSE 0 END) AS OpRecQty,
           SUM(CASE WHEN VT.NCAT IN ('PRR','PRC','RQR','KSISS','KMISS')
                    THEN S.QtyIss * ISNULL(S.ConvRatio, 1) ELSE 0 END) AS OpIssQty,
           SUM(CASE WHEN VT.NCAT IN ('PBC','PBR','STOP','MRE','RQI','KSREC','KMREC')
...
Period AS (
    SELECT S.Item,
           SUM(CASE WHEN VT.NCAT IN ('PBC','PBR','MRE','RQI','STOP','KSREC','KMREC')
                    THEN S.QtyRec * ISNULL(S.ConvRatio, 1) ELSE 0 END) AS RecQty,
           SUM(CASE WHEN VT.NCAT IN ('PRR','PRC','RQR','STOP','KSISS','KMISS')
                    THEN S.QtyIss * ISNULL(S.ConvRatio, 1) ELSE 0 END) AS IssQty,
           SUM(CASE WHEN VT.NCAT IN ('PBC','PBR','MRE','RQI','STOP','KSREC','KMREC')

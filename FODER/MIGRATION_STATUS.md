...
│   ├── pos_masters.py               # POS session/scheme/delboy/itemcat (COMPLETE)
│   ├── hr_masters.py                # HR category/holiday/employee (COMPLETE)
│   ├── members_masters.py           # Member category/facility (COMPLETE)
│   ├── general_setup.py             # Room feature/godown/vouchertype/enviro/guestparam (COMPLETE)
...
│   ├── hr_members_ui.py             # HR + Members masters (COMPLETE)
│   ├── general_setup_ui.py          # General setup masters (COMPLETE)
...
| core/checkout.py | FdCheckOut, FdReSetlement | ✅ COMPLETE | 100% | Checkout + reverse + balance |
...
| **INVENTORY (Phase 7)** |
| core/inventory.py | Godown, PIndent, pMREntry | ✅ COMPLETE | 100% | Godown CRUD + Indent/GIN/Stock entry |
...
| **FrmControlPanel** | Control panel - system settings | HIGH |
...
| **MemberModule** | Member operations (visit, renewal, billing) | MEDIUM |
| **SmartCardRegistration / Recharge / Refund / LostReIssue** | Smart card operations | LOW |
| **FrmTallyExport** | Tally accounting export | MEDIUM |
...
|---|---|---|---|
| BUG-001 | core/area.py | HIGH | CityCode widened from 5→6 via migration; verify all FK references |
| BUG-002 | core/roomstatus.py | MEDIUM | RoomBLockOut table empty - Maintenance/OOO blocked |
| BUG-003 | core/nightaudit.py | MEDIUM | NA posting not implemented (KOTAtNightAudit, POSBillAtNightAudit) |
| BUG-004 | core/pos.py | MEDIUM | KOT entry not implemented (READ-ONLY only) |
| BUG-005 | core/inventory.py | MEDIUM | Purchase/Indent/Stock docs READ-ONLY |
| BUG-006 | ui/shell.py | LOW | Keyboard shortcuts Alt+1..9 only (no Alt+10+) |
| BUG-007 | core/db.py | LOW | INI path hardcoded (C:\Drive\HMS2526\Analysis.ini) |
| BUG-008 | core/auth.py | LOW | SA default password handling (stored as '.') |
| BUG-009 | All core modules | MEDIUM | Hardcoded SITE_CODE='KK', USER='PYADMIN' |
| BUG-010 | core/reservation.py | MEDIUM | Vprefix hardcoded to current year; should be configurable |

...
-- --------------------------------------------
SELECT Code,Name FROM Employee
    Where (LogSite_Code='<LogSiteCode>' or T.LOGSITE_CODE='HO') AND T.Type in ('RO') and RoomOcc.ChkoutDate is NULL
    Order By T.Code;
...
Select SubCode as SearchCode,Name From SubGroup
    ...
    Where RestType In ('Outlet','Room Service'))) And (LOGSITE_CODE='<LogSiteCode>' or LOGSITE_CODE='HO')
    Order by Name;
...
-- Uses table: SubGroup
-- --------------------------------------------
SELECT P.VNO AS RECTNO,P.VDATE AS RECTDATE,P.PAYTYPE AS RECTMODE,P.U_Name As UserName,P.AMTCR AS RECTAMT,P.CardNo AS CCardNo,P.Cardholder As CCardholder,P.ChqNo,P.ChqDate,P.ExpDate AS CCExpDate,P.BatchNo,P.AgAc,S.Name AS AgCompany, R.NAME As PayName,P.TxnNo FROM PAYCHARGE P
    LEFT JOIN RevMast R ON P.PayCode = R.Code
    LEFT JOIN SubGroup S ON P.AgAc = S.SubCode
    WHERE P.DOCID=''
    Order By Cast(LTrim(Substring(DocId2,14,8)) As Integer);
...
Select Docid,GuestProf from GuestFolio
    where LOGSITE_CODE='<LogSiteCode>' AND Docid='<Docid><Param>;

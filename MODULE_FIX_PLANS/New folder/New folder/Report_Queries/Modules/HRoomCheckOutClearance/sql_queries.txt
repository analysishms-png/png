...
Select * from roommast R
    ...
    where (R.LOGSITE_CODE='<LogSiteCode>' or R.LOGSITE_CODE='HO') and R.Type='RO' AND R.InclCount='Y' and O.Type Not IN ('O','C')
    order by COde;
...
-- Uses table: RoomOcc
-- --------------------------------------------
Select R.DocId,Max(R.RoomNo) RoomNo,Max(IsNull(P.Bill_No,'')) Bill_No,Max(IsNull(RC.ChkOutClearanceYN,'')) CHKOUTCLYN from RoomOcc R
    Left Join PayCharge P On P.FolionoDocID=R.DocId Or P.RelatedFolioNoDocId=R.DocId
    Left Join RoomCheckOutStatus RC On R.DocId=RC.FolionoDocid And R.RoomNo=RC.RoomCode
    where (R.LOGSITE_CODE='<LogSiteCode>') and ChkOutDate is Null
    Group By R.DocId;
...
-- Table [RoomCheckOutStatus]  NOT present in visahl.sql dump (schema unknown)

...
| B023 | core/tds.py | Duplicate module — tdscat.py already existed (same TDSCat CRUD); dedup rule violate hua | Closed — tds.py deleted; ui/test tdscat par redirected; tdscat.insert me require_absent guard bhi add (batch me chhut gaya tha) |
| B024 | core/reports.py | Parallel pass ne 231-report version merge kiya jisme B019 literal-? fix nahi tha — detailed_trial family phir se "odd date params" | Closed — B019 fix reapplied run() me (literal-strip regex) |
| B025 | core/tally_export.py | Acgroup me 2 empty MainGrCode ('') rows aayi — LEN-3 negative SUBSTRING crash + CAST('' AS NUMERIC) 8114; Tally export 0/22 | Closed — CASE LEN>3 guard + ISNUMERIC order-by fallback 999999; 22/22 PASS |
| B026 | core/reports.py + tools/_fix_s6_batch1.py | v1 fixer ka greedy regex multi-line "sql": ( blocks skip karke target SQL 2 door blocks (NCKOTSummary, Reconciliation(R2A)) me cross-write kar gaya; targets untouched | Closed — v2 bounded regenerator (block = key-line -> next key-line), 23 entries + 2 victims REPORTS_TXT docs se restore, post-scan CLEAN; Inventory 13->33/33; lesson: bulk-edit hamesha bounded + post-scan ke saath |
